...
import {
  ...
  Barcode, RefreshCw, X, ArrowRight, ArrowLeft, Sparkles, Tag, ShoppingBag, Layers, Calculator
} from 'lucide-react';
import { apiRequest } from '../api';

export default function POS({ showToast, onSaleComplete }) {
  const [mobileView, setMobileView] = useState('cart'); // 'cart' or 'checkout'
  ...
  return (
    <div className="h-[calc(100dvh-53px-56px)] lg:h-[calc(100vh-53px)] flex flex-col overflow-hidden bg-slate-100 font-sans">
      {/* 1. TOP CASHIER COMMAND STRIP */}
      <div className="bg-slate-900 border-b border-slate-800 px-3 sm:px-4 py-1.5 flex items-center justify-between text-xs text-slate-300 shrink-0">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="flex items-center gap-1.5 font-bold text-emerald-400">
            <Barcode className="w-4 h-4 text-emerald-400" />
            <span className="uppercase tracking-wider text-[11px]">POS Terminal</span>
          </div>
          <span className="text-slate-700 hidden sm:inline">|</span>
          <div className="hidden sm:flex items-center gap-2 text-[11px] text-slate-400">
            <span>Shortcuts:</span>
            <span className="flex items-center gap-1"><kbd className="kbd-chip kbd-chip-dark text-emerald-300">F2</kbd> Pay</span>
            <span className="flex items-center gap-1"><kbd className="kbd-chip kbd-chip-dark">F4</kbd> Search</span>
            <span className="flex items-center gap-1"><kbd className="kbd-chip kbd-chip-dark">F8</kbd> Hold</span>
            <span className="flex items-center gap-1"><kbd className="kbd-chip kbd-chip-dark">ESC</kbd> Clear</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {heldBills.length > 0 && (
            <button
              ...
              className="bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/40 px-2 sm:px-2.5 py-0.5 rounded-lg text-[11px] font-bold flex items-center gap-1.5 transition cursor-pointer"
            >
          ...
          <div className="flex items-center gap-1 text-[11px] font-mono bg-slate-800/80 px-2 py-0.5 rounded border border-slate-700/60 text-slate-300">
            <span className="text-slate-400 hidden xs:inline">Cart:</span>
            <span className="font-bold text-emerald-400">{cart.length} items</span>
          </div>
        </div>
      </div>

      {/* Mobile Step Switcher (Products vs Checkout) */}
      <div className="md:hidden flex border-b border-slate-200 bg-white px-2.5 py-1.5 gap-2 shrink-0 select-none">
        <button
        ...
        >
          ...
          <span>2. Pay (₹{grandTotal.toFixed(0)})</span>
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <div className={`flex-1 flex-col p-2.5 sm:p-3.5 space-y-2.5 sm:space-y-3 overflow-hidden border-r border-slate-200/80 ${mobileView === 'cart' ? 'flex' : 'hidden md:flex'}`}>
          ...
          {/* Mobile Quick Action to Proceed to Pay */}
          {cart.length > 0 && (
            <div className="md:hidden shrink-0 pt-1 pb-1">
              <button
                onClick={() => setMobileView('checkout')}
                className="w-full bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-600 text-white font-extrabold py-3 px-4 rounded-xl flex items-center justify-between shadow-lg shadow-emerald-950/40 active:scale-98 transition cursor-pointer"
              >
                <div className="text-left">
                  <p className="text-[10px] text-emerald-100 uppercase tracking-wider font-semibold">{cart.length} items ready</p>
                  <p className="text-base font-black font-mono">Total: ₹{grandTotal.toFixed(2)}</p>
                </div>
                <div className="flex items-center gap-1.5 text-xs font-bold bg-white/20 px-3 py-1.5 rounded-lg">
        ...
        <div className={`w-full md:w-[380px] bg-white border-l border-slate-200 p-3.5 sm:p-4 flex-col justify-between overflow-y-auto select-none ${mobileView === 'checkout' ? 'flex' : 'hidden md:flex'}`}>
          <div className="space-y-3.5">
            {/* Mobile Header: Back to Products button */}
            <div className="md:hidden flex items-center justify-between pb-2 mb-1 border-b border-slate-200">
              <button
                onClick={() => setMobileView('cart')}
                className="text-xs font-bold text-slate-700 hover:text-slate-900 flex items-center gap-1.5 py-1.5 px-3 rounded-lg bg-slate-100 border border-slate-200 cursor-pointer"
              >
                <ArrowLeft className="w-3.5 h-3.5" />
                <span>← Back to Products ({cart.length})</span>
              </button>
              <span className="text-xs font-black font-mono text-emerald-600">₹{grandTotal.toFixed(2)}</span>
            </div>
