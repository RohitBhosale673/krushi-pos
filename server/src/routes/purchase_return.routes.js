...
router.get('/', requirePermission('returns', 'purchase_return'), (req, res) => {
...
router.post('/', requirePermission('returns', 'purchase_return'), (req, res) => {
  ...
  try {
    ...
    transaction(() => {
      ...
      const countRow = queryOne('SELECT COUNT(*) AS total FROM purchase_returns');
      newReturnNo = `PR/${new Date().getFullYear()}/${String(countRow.total + 1).padStart(4, '0')}`;
      const debitNoteNo = `DN/${new Date().getFullYear()}/${String(countRow.total + 1).padStart(4, '0')}`;
      ...
      for (const item of processedItems) {
        ...
        const batch = queryOne('SELECT available_qty FROM product_batches WHERE id = ?', [item.batch_id]);
        const newAvail = Math.max(0, batch.available_qty - item.qty);

        run('UPDATE product_batches SET available_qty = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [newAvail, item.batch_id]);
