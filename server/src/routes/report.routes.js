...
router.get('/dashboard', requirePermission('reports', 'view'), (req, res) => {
  ...
  const totalUdhari = queryOne("SELECT COALESCE(SUM(current_balance), 0) AS val FROM customers WHERE current_balance > 0")?.val || 0;

  const inventoryValuation = queryOne(`
    SELECT 
  ...
  const totalPurchases = queryOne(`SELECT COALESCE(SUM(grand_total), 0) AS val FROM purchases WHERE ${purchaseDateCondition}`, queryParams)?.val || 0;
  const totalExpenses = queryOne(`SELECT COALESCE(SUM(amount), 0) AS val FROM expenses WHERE ${expenseDateCondition}`, queryParams)?.val || 0;
...
router.get('/todays-sales', requirePermission('reports', 'view'), (req, res) => {
  ...
  const cashierBreakdown = queryAll(`
...
router.get('/sales', requirePermission('reports', 'view'), (req, res) => {
  ...
  const totals = queryOne(`
    ...
    FROM sales s
    LEFT JOIN customers c ON s.customer_id = c.id
    WHERE 1=1 ${start_date ? "AND DATE(s.sale_date) >= '" + start_date + "'" : ""} ${end_date ? "AND DATE(s.sale_date) <= '" + end_date + "'" : ""}
  `);
...
router.get('/profit-loss', requirePermission('reports', 'view'), (req, res) => {
...
router.get('/export/:type', requirePermission('reports', 'export'), (req, res) => {
