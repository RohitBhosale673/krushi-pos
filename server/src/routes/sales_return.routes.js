...
router.post('/', requirePermission('returns', 'sales_return'), async (req, res) => {
  ...
  try {
    const sale = await queryOne('SELECT * FROM sales WHERE invoice_no = ?', [original_invoice_no]);
    if (!sale) {
    ...
    await transaction(async () => {
      ...
      const countRow = await queryOne('SELECT COUNT(*) AS total FROM sales_returns');
      newReturnNo = `SR/${new Date().getFullYear()}/${String((countRow?.total || 0) + 1).padStart(4, '0')}`;
      ...
      for (const item of processedReturnItems) {
        ...
        if (item.condition_type === 'Restockable') {
          const batch = await queryOne('SELECT available_qty, qty_sold FROM product_batches WHERE id = ?', [item.batch_id]);
          const newAvail = batch.available_qty + item.qty;
          const newSold = Math.max(0, batch.qty_sold - item.qty);

          await run('UPDATE product_batches SET available_qty = ?, qty_sold = ?, status = "Active", updated_at = CURRENT_TIMESTAMP WHERE id = ?', [newAvail, newSold, item.batch_id]);
      ...
      if (sale.customer_id && (refund_method === 'Credit Adjustment' || sale.due_amount > 0)) {
        ...
        const newBal = Math.max(0, (cust?.current_balance || 0) - totalReturnAmt);
